...
function Home({ search, setSearch, watchlist }) {
  ...
  useEffect(() => {
    ...
    getPopularMovies(page).then((data) => {
      setpopularMovies((prevMovies) => {
        // FIX: deduplicate by movie id to prevent duplicate key warnings
  ...
  const displayedMovies =
    search.trim() === "" ? popularmovies : (searchResults || []);
    ...
    <>
      ...
      <div>
        <Navbar
          ...
          search={search}
          setSearch={setSearch}
          setShowGenrePanel={setShowGenrePanel}
        />

        {showGenrePanel && (
          <div className="genre-overlay" onClick={() => setShowGenrePanel(false)}>
            <motion.div
            ...
            >
              ...
              <div className="genre-grid">
                {genres?.map((genre) => (
                  <motion.button
                    key={genre.id}  // FIX: added missing key prop
            ...
            <div className="overlay-bg" onClick={() => setShowGenrePanel(false)}></div>
          </div>
        )}
        ...
        <div className="movies-grid">
          {loadingSkelton ? (
            ...
            filteredMovie.map((movie) => (
              <motion.div
                key={movie.id}  // FIX: use stable movie.id instead of array index
